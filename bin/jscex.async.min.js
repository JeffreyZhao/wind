...
(function(){Jscex.builders.async={binder:"$await",Start:function(a,c){return new Jscex.Async.Task({start:function(b){c.start(a,function(a,f){if(a=="normal"||a=="return")b("success",f);else if(a=="throw")b("error",f);else throw"Unsupported type: "+a;})}})},Bind:function(a,c){return{start:function(b,d){function f(f){if(f.status=="succeeded"){var e;try{e=c.call(b,a.result)}catch(g){d("throw",g);return}e.start(b,d)}else d("throw",a.error)}a.status=="ready"?(a.addListener(f),a.start()):a.status=="running"?
